...
class EncoderLayer(nn.Module):
    ...
    def __call__(self, x, en_mask):
        ...
        attention = SelfAttention(features=self.features, heads=self.heads)(inputs=lnx, mask=en_mask)
...
def train(batch_size: int, vocab_size: int, max_seq_len: int, embed_size: int, heads: int,
    ...
    train_loader, valid_loader, test_loader = load_dataset(batch_size=batch_size, max_seq_len=max_seq_len)
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append(f'Epoch: [{epoch:>3d}/{epochs}], train loss: {train_loss:>6.4f}, train acc: {train_acc:>5.2f}%, '
                       f'test acc: {test_acc:>5.2f}%, etc: {toc - tic:>5.2f}s')
        print(history[-1])
...
if __name__ == '__main__':
    ...
    train(batch_size=2, vocab_size=257, max_seq_len=4000, embed_size=512, heads=8, n_layer=6,
