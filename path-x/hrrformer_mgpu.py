...
def train(batch_size: int, vocab_size: int, max_seq_len: int, embed_size: int, heads: int,
    ...
    train_loader, _, test_loader = load_dataset(batch_size=batch_size, resolution=128, n_devices=1, split='hard')
    ...
    tx = optax.adamw(learning_rate=scheduler, weight_decay=5e-4)
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append(f'Epoch: [{epoch:>2d}/{epochs}], train loss: {train_loss:>8.4f}, '
...
if __name__ == '__main__':
    train(batch_size=16, vocab_size=256, max_seq_len=128 * 128, embed_size=1024, heads=8, n_layer=1,
