...
class EncoderLayer(nn.Module):
    ...
    @nn.compact
    def __call__(self, inputs, en_mask):
        attention = SelfAttention(features=self.features, heads=self.heads)(inputs=inputs, mask=en_mask)
        attention = nn.Dropout(self.dropout_rate, deterministic=not self.training)(attention)
        x = inputs + attention

        lnx = nn.LayerNorm(epsilon=1e-6)(x)
        ffn = MLP(self.features, self.dropout_rate, training=self.training)(lnx)
        return x + ffn
...
class Network(nn.Module):
    ...
    def __call__(self, inputs1, inputs2, training: bool = False):
        ...
        mask1 = np.where(inputs1 > 0, 1., 0.)
        mask1 = mask1[:, np.newaxis, :, np.newaxis]

        mask2 = np.where(inputs2 > 0, 1., 0.)
        mask2 = mask2[:, np.newaxis, :, np.newaxis]

        embedding = Embedding(vocab_size=self.vocab_size,
                              embed_size=self.embed_size,
                              max_seq_len=self.max_seq_len)
        dropout = nn.Dropout(self.dropout_rate, deterministic=not training)
        layer_norm = nn.LayerNorm(epsilon=1e-6)

        encoders = []
        for _ in range(self.n_layer):
            encoders.append(EncoderLayer(features=self.embed_size,
                                         heads=self.heads,
                                         dropout_rate=self.dropout_rate,
                                         training=training))

        x1 = embedding(inputs1)
        ...
        for block in encoders:
            x1, x2 = (block(x1, en_mask=mask1), block(x2, en_mask=mask2))
...
def initialize_model(model, max_seq_len, init_rngs):
    ...
    variables = model.init(init_rngs, *(init_inputs, init_inputs))['params']
    return variables
...
def train(batch_size: int, vocab_size: int, max_seq_len: int, embed_size: int, heads: int,
    ...
    train_loader, valid_loader, test_loader = load_dataset(batch_size=batch_size, max_seq_len=max_seq_len, n_devices=16)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for data in train_loader:
            ...
            x_true1, x_true2, y_true = data['inputs1'], data['inputs2'], data['targets']
        ...
        for data in valid_loader:
            ...
            valid_acc.append(metrics['acc'].mean())

        valid_acc = sum(valid_acc) / len(valid_acc) * 100
        ...
        history.append(f'Epoch: [{epoch:>3d}/{epochs}], train loss: {train_loss:>6.4f}, train acc: {train_acc:>5.2f}%, '
                       f'valid acc: {valid_acc:>5.2f}%, test acc: {test_acc:>5.2f}%, etc: {toc - tic:>5.2f}s')
        print(history[-1])
...
if __name__ == '__main__':
    ...
    train(batch_size=4, vocab_size=257, max_seq_len=4000, embed_size=128, heads=4, n_layer=4,
          output_size=2, dropout_rate=0.1, lr=1e-3, epochs=20)
